...
# @Time : 2022/2/24 16:46
# @Auth: GLK
# @File: test_ioc_config.py
# @Software :PyCharm
...
from api.ioc_config_api import Api_Ioc_config

mylogger = Logger(logger="test_ioc_config_api").getlog()

actual_code_list = []
actual_status_list = []
result_list = []
path = BASE_DIR + "/database/ioc_config.xlsx"


class Test_Api_Ioc_config(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        mylogger.info("开始测试Test_ioc_config_api")
        cls.ioc_config_api = Api_Ioc_config()
    ...
    def test_ioc_config_api(self, case_name, api, request_body, headers, method, expect_code, status_code):
        ...
        response = self.ioc_config_api.test_ioc_config_api(url=url, json=request_body, headers=headers,
                                                 method=method)
